...
limit_readings = np.array([joint[0].receive_pdo_2(joint[1])[0] for joint in robot.joints]) * joint_axis_directions
print([f"{reading:.2f}" for reading in limit_readings])

while robot.command_controller.commands.get("mode_switch") != 1:
    joint_readings = np.array([joint[0].receive_pdo_2(joint[1])[0] for joint in robot.joints]) * joint_axis_directions
...
with open("calibration_2.yaml", "w") as f:
